...
if (y0==startyear){
 ...
 # load input table
  data_mun <- readr::read_rds("_Crosswalk_pre.rds")
...
## save final data
saveRDS(data_mun,paste0("_Crosswalk_final_",startyear,"_",endyear,".rds"))
...
# data_mun_sf <- left_join(data_mun, map, by=c('name_muni', 'code_muni_2010' = 'code_muni' ))

# summarize to dissolve borders by code_amc
...
ggplot() + geom_sf(data = data_mun_sf)

# Save cleaned sf in the cleaned directory
file.name <- paste0(paste0("AMC_",startyear,"_",endyear),".gpkg")
...
##### Exemplo com funcao
system.time(
amc_table <- table_amc(startyear = 1980, endyear = 2010) # ok
)
...
amc_table2 <- subset(amc_table, !is.na(amc))

m1970 <- geobr::read_municipality( year= 1970)
m1991 <- geobr::read_municipality( year= 1991)
m1980 <- geobr::read_municipality( year= 1980)
m2010 <- geobr::read_municipality( year= 2010)

setdiff(amc_table2$code2010, m1970$code_muni)
setdiff(m1970$code_muni, amc_table2$code2010)



geobr::read_municipality( year= 2010, code_muni = 2306405)

subset(amc_table2, code2010 == 2306405)

aaaa <- subset(data_mun, code2010 == 2306405)


subset(m1970, name_muni == 'Itapipoca')
subset(m1991, name_muni == 'Itapipoca')
subset(m1980, name_muni == 'Itapipoca')
subset(m2010, name_muni == 'Itapipoca')
