...
malhas_municipais <- function(region=NULL){
  ...
  if (region == "meso_regiao"){all_shapes <- all_shapes[all_shapes %like% "mesorregioes|me"]}
  if (region == "micro_regiao"){all_shapes <- all_shapes[all_shapes %like% "microrregioes|mi"]}
  if (region == "municipio"){all_shapes <- all_shapes[all_shapes %like% "municipios|mu500|mu2500|mu1000"]}
  ...
  shp_to_sf_rds <- function(x){
    ...
    if ( last15(x) %like% "UF|uf|ME|me|MI|mi|MU|mu"){ dest_dir <- paste0("./shapes_in_sf_all_years_original/",region,"/", year)}
