...
read_municipio <- function(ano=NULL, cod_mun=NULL){

  if(is.null(ano) || is.null(cod_mun)){ #testa se os parametros são nulos;
    stop("Error: Invalid value to argument ano or argument cod_mun.") 
  }
  if(ano %in% str_extract(list.files(root_dir, pattern = ".*\\_"), pattern = "[0-9]+")){ #testa se o ano existe;
    if(substr(x = cod_mun, 1, 2) %in% substr(list.files(paste0(root_dir, "\\MU_", ano)), 1, 2)){ #testa se o estado existe;
    
   shape <- readRDS(paste0(root_dir, "\\MU_", ano, "\\", substr(x = cod_mun, 1, 2), "MU.rds")) 
